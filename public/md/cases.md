...
## Part 8: Critical User Flows (Added 2026-02-12)

**Bug Reference:** "Failed to fetch case" error after form submission  
**Root Cause:** next.config.js had broken API rewrite to localhost:4000

### 8.1 Create → Fetch Flow

| # | Step | Expected | Verified |
|---|------|----------|----------|
| CF01 | POST /api/cases with valid data | Returns 201 with case ID | ✅ |
| CF02 | Response includes `id` field | ID is non-empty string | ✅ |
| CF03 | GET /api/cases/[id] with returned ID | Returns 200 with case data | ✅ |
| CF04 | Case data matches POST data | All fields preserved | ✅ |
| CF05 | impactedAreas parsed from JSON | Returns array, not string | ✅ |

### 8.2 Participant Flow

| # | Step | Expected | Verified |
|---|------|----------|----------|
| CF06 | POST /api/cases/[id]/participants | Returns participant with token | ✅ |
| CF07 | GET /api/cases/[id] includes participants | Array with surveyUrl | ✅ |
| CF08 | surveyUrl contains token | URL format /survey/[token] | ✅ |

### 8.3 Survey Flow

| # | Step | Expected | Verified |
|---|------|----------|----------|
| CF09 | GET /api/survey/[token] | Returns questions + context | ✅ |
| CF10 | POST /api/survey/[token]/responses | Saves responses | ✅ |
| CF11 | Status updates to COMPLETED | Participant status changes | ✅ |
| CF12 | firstResponseAt set on case | Timestamp recorded | ✅ |

### 8.4 Results Flow

| # | Step | Expected | Verified |
|---|------|----------|----------|
| CF13 | GET /api/cases/[id]/results | Returns ICS + recommendation | ✅ |
| CF14 | ICS in range 0-100 | Numeric, bounded | ✅ |
| CF15 | Dimension scores present | D1-D5 all present | ✅ |

### 8.5 Next.js Route Verification

| # | Check | Expected | Verified |
|---|-------|----------|----------|
| CF16 | /api/cases/[id] route exists | Dynamic route file present | ✅ |
| CF17 | No external API rewrites | next.config.js clean | ✅ |
| CF18 | Routes handled by Next.js | No proxy to localhost:4000 | ✅ |
...
```bash
# Run all 329 automated tests
npm test

# Run specific test file
npx vitest run tests/scoring.test.ts

# Run critical flows specifically
...
*Updated: 2026-02-12 - Added critical flow tests after "Failed to fetch case" bug*
