        ...
        <div className="p-6 lg:p-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-brand-grey text-sm mt-1">Manage platform users and their access</p>
            </div>
            <Button>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    statusFilter === status
                      ...
                      : 'bg-white/5 text-brand-grey border border-white/10 hover:bg-white/10'
                  }`}
                >
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4">
              ...
              <p className="text-sm text-brand-grey">Active Users</p>
            </div>
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
              <p className="text-2xl font-bold text-amber-400">{USERS.filter(u => u.status === 'pending').length}</p>
              <p className="text-sm text-brand-grey">Pending</p>
            </div>
            <div className="bg-gray-500/10 border border-gray-500/20 rounded-xl p-4">
              <p className="text-2xl font-bold text-brand-grey">{USERS.filter(u => u.status === 'inactive').length}</p>
              <p className="text-sm text-brand-grey">Inactive</p>
            </div>
          </div>
          ...
          <div className="bg-white/[0.02] backdrop-blur-sm rounded-xl border border-white/[0.06] overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="border-b border-white/[0.06]">
                  <tr>
                    <th className="text-left p-4 text-sm font-medium text-brand-grey">User</th>
                    <th className="text-left p-4 text-sm font-medium text-brand-grey">Organization</th>
                    <th className="text-left p-4 text-sm font-medium text-brand-grey">Role</th>
                    ...
                    <th className="text-left p-4 text-sm font-medium text-brand-grey">Last Active</th>
                    ...
                    <tr key={user.id} className="hover:bg-white/[0.02]">
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-end gap-2">
                          <button className="p-2 text-brand-grey hover:text-white hover:bg-white/5 rounded-lg">
                            <Mail className="w-4 h-4" />
                          </button>
                          <button className="p-2 text-brand-grey hover:text-white hover:bg-white/5 rounded-lg">
                            <Edit className="w-4 h-4" />
                          </button>
                          <button className="p-2 text-brand-grey hover:text-red-400 hover:bg-red-500/10 rounded-lg">
            ...
            {filteredUsers.length === 0 && (
              <div className="p-8 text-center text-brand-grey">
          ...
          <div className="mt-4 flex items-center justify-between">
            <p className="text-sm text-brand-grey">Showing {filteredUsers.length} of {USERS.length} users</p>
            <div className="flex gap-2">
