        ...
        <div className="p-6 lg:p-8 max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-brand-grey text-sm mt-1">Manage assessment questions by dimension</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-4">
              ...
              <p className="text-sm text-brand-grey">Total Questions</p>
            </div>
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-4">
              <p className="text-2xl font-bold">{DIMENSIONS.length}</p>
              <p className="text-sm text-brand-grey">Dimensions</p>
            </div>
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-4">
              <p className="text-2xl font-bold">{QUESTIONS.filter(q => q.isReverse).length}</p>
              <p className="text-sm text-brand-grey">Reverse Scored</p>
            </div>
          </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-sm text-brand-grey">{questions.length} questions</p>
                    </div>
                  </div>
                  {expandedDimension === dimension ? <ChevronUp className="w-5 h-5 text-brand-grey" /> : <ChevronDown className="w-5 h-5 text-brand-grey" />}
                </button>
                      ...
                      <div key={q.id} className={`p-4 flex items-start gap-4 ${idx !== questions.length - 1 ? 'border-b border-white/[0.06]' : ''}`}>
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="px-2 py-0.5 rounded bg-white/5 text-xs font-mono text-brand-grey">{q.code}</span>
                            <span className="px-2 py-0.5 rounded bg-brand-green/10 text-xs text-brand-green">{q.role}</span>
                          ...
                          <div className="flex gap-1 mt-2">
                            {q.variants.map(v => (
                              <span key={v} className="px-2 py-0.5 rounded bg-white/5 text-xs text-brand-grey">{v}</span>
                            ))}
                          </div>
                        </div>
