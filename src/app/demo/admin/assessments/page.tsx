          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <div className="flex gap-2">
              {['all', 'DRAFT', 'ACTIVE', 'COMPLETED'].map(status => (
                <button key={status} onClick={() => setStatusFilter(status)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${statusFilter === status ? 'bg-brand-green/20 text-brand-green border border-brand-green/30' : 'bg-white/5 text-brand-grey border border-white/10 hover:bg-white/10'}`}>
          ...
          <div className="bg-white/[0.02] backdrop-blur-sm rounded-xl border border-white/[0.06] overflow-hidden">
            <table className="w-full">
              <thead className="border-b border-white/[0.06]">
                <tr>
                  <th className="text-left p-4 text-sm font-medium text-brand-grey">Assessment</th>
                  <th className="text-left p-4 text-sm font-medium text-brand-grey">Variant</th>
                  <th className="text-left p-4 text-sm font-medium text-brand-grey">Status</th>
                  <th className="text-left p-4 text-sm font-medium text-brand-grey">Progress</th>
                  <th className="text-left p-4 text-sm font-medium text-brand-grey">ICS</th>
                  <th className="text-left p-4 text-sm font-medium text-brand-grey">Result</th>
                ...
                {filteredAssessments.map(assessment => (
                  <tr key={assessment.id} className="hover:bg-white/[0.02]">
                    <td className="p-4">
                      ...
                      <p className="text-xs text-brand-grey">{assessment.organization} • {assessment.createdAt}</p>
                    </td>
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        assessment.variant === 'QUICK' ? 'bg-brand-green/10 text-brand-green' :
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium ${
                        ...
                        assessment.status === 'ACTIVE' ? 'bg-brand-green/10 text-brand-green' :
                    ...
                    <td className="p-4">
                      <div className="flex items-center justify-end gap-2">
                        <button className="p-2 text-brand-grey hover:text-white hover:bg-white/5 rounded-lg"><Eye className="w-4 h-4" /></button>
